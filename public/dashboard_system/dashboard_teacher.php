...
<?php 
$total_students = query("SELECT COUNT(*) AS count 
FROM student_grades sg
LEFT JOIN schedule s ON sg.schedule_id = s.schedule_id
WHERE s.teacher_id = ? AND s.syid = ?", 
$_SESSION['sunbeam_app']['userid'], $sy['syid']); 
?>
<?php 
$total_subjects = query("SELECT COUNT(DISTINCT subject_id) AS count 
                         FROM schedule 
                         WHERE teacher_id = ? AND syid = ?", 
                         $_SESSION["sunbeam_app"]["userid"], $sy["syid"]); ?>
<?php $total_advisory = query("select count(*) as count from advisory where teacher_id = ? and school_year = ?", $_SESSION["sunbeam_app"]["userid"], $sy["syid"]); ?>
    <!-- Main content -->
      ...
      <div class="row">
          <div class="col-lg-4 col-6">
            ...
            <div class="small-box bg-info">
              ...
              <a href="teacherAdvisory" class="small-box-footer">More info <i class="fas fa-arrow-circle-right"></i></a>
            </div>
          </div>
          <!-- ./col -->
